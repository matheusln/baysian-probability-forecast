...
pkgs<-c("tidyverse", "readxl", "openxlsx", "jsonlite")
...
```{r}
dir.agendas = "base de dados/agenda agosto 2024.xlsx"
abas        = excel_sheets("base de dados/agenda agosto 2024.xlsx")

agenda = abas |>
         map_df(~ read_xlsx(dir.agendas, sheet = .x,) |>
                  select(1:13)                        |>
...
remove(abas, agenda, converter_horas, dir.agendas, processar_horas)
```

```{r}
#tibble(Ncanc = as.numeric(table(agenda.h$cancelada)),
#       Pcanc = as.numeric(prop.table(table(agenda.h$cancelada))*100),
#       Nrecu = as.numeric(table(agenda.h$recusada)),
#       Precu = as.numeric(prop.table(table(agenda.h$recusada))*100)) 

agenda.2 = agenda.1 |>
           filter(cancelada == 0 & recusada == 0) |>
           select(-c(CLIENTE, PRIORIDADE, cancelada, recusada))
...
```{r}
trafegus = read.csv("base de dados/trafegus.csv", stringsAsFactors = FALSE)                            |>
...
trafegus.5 = trafegus.4                                         |>
             filter(!is.na(viag_numero_manifesto ) & 
                    !is.na(placa_cavalo)           &
                    !is.na(placa_carreta))                      |>
             select(-c(X,pess_nome_transportador,
                       nome_transportador, 
...
```{r}
rastreador = read.table("base de dados/macros.txt", header = TRUE, sep = ";")    |>
...
df.4 = df.3 |>
       ...
       select(-c(hora_chegada_cliente_1, hora_chegada_cliente_2, hora_chegada_cliente_3,
                 ...
                 menor_diferenca, hora_descarga_agenda))                               |>
       mutate(diferenca_agenda_chegada = (as.numeric(difftime(hora_chegada_cliente, hora_carga_agenda, units = "secs"))/60)/60, 
              tranportador_atrasou = as.factor(ifelse(diferenca_agenda_chegada > 0, 1, 0)), .after = 4)     |>
       ...
       mutate(diferenca_agenda_carreg = (as.numeric(difftime(hora_inicio_carreg, hora_carga_agenda, units = "secs"))/60)/60, 
              cliente_atrasou = as.factor(ifelse(diferenca_agenda_carreg > 0, 1, 0)), .after = 7) |>
       ...
       select(-c(hora_inicio_viag_1:hora_fim_viagem_6, id_data_carga.y))
...
df.5 = df.3 |>
       ...
       mutate(diferenca_agenda_chegada = (as.numeric(difftime(hora_chegada_cliente, hora_descarga_agenda, units = "secs"))/60)/60, 
              tranportador_atrasou = as.factor(ifelse(diferenca_agenda_chegada > 0, 1, 0)), .after = 2) |>
       ...
       mutate(diferenca_agenda_descarreg = (as.numeric(difftime(hora_inicio_descarreg, hora_descarga_agenda, units = "secs"))/60)/60, 
              cliente_atrasou = as.factor(ifelse(diferenca_agenda_descarreg > 0, 1, 0)), .after = 4) |>
...
```{r}
data.frame(
  colunas = names(colSums(is.na(df.3))),
...
atraso.carreg.transp = df.4 |> 
  select(-c(id_PLACA.y, # poucas observações para manter cada ativo
            id_PLACA.x, # variável repetidas por causa dos joins
            hora_chegada_cliente, # manter apenas data e hora da agenda
            hora_inicio_carreg,   # baser para contruir a variável de atraso do cliente
            diferenca_agenda_carreg, # relevante para base de atraso do cliente
            cliente_atrasou, # relevante para base de atraso do cliente
            hora_fim_carreg, # baser para contruir a variável delta_carregamento
            parado_ou_viagem,# não trouxe mais informações
            id_data_carga.y, # repedita por causa do join
            cpf_moto,   # poucas observações para manter cada motorista
            id_CPF_MOTO # repedita por causa do join
            )) |>
  filter(!is.na(GR_JORNADA)) |>
  select(tranportador_atrasou, everything()) |>
  mutate(numero_paradas.rast = numero_paradas.x,
         numero_paradas.gris = numero_paradas.y,
         intraestado = as.factor(intraestado),
         interestado = as.factor(interestado),
...
atraso.carreg.cliente = df.4 |> 
  select(-c(id_PLACA.y, # poucas observações para manter cada ativo
            id_PLACA.x, # variável repetidas por causa dos joins
            hora_chegada_cliente, # manter apenas data e hora da agenda
            hora_inicio_carreg,   # baser para contruir a variável de atraso do cliente
            diferenca_agenda_chegada, # relevante para base de atraso do transportador
            tranportador_atrasou, # relevante para base de atraso do transportador
            hora_fim_carreg, # baser para contruir a variável delta_carregamento
            parado_ou_viagem,# não trouxe mais informações
            id_data_carga.y, # repedita por causa do join
            cpf_moto,   # poucas observações para manter cada motorista
  ...
  filter(!is.na(GR_JORNADA)) |>
  select(cliente_atrasou, everything()) |>
  mutate(numero_paradas.rast = numero_paradas.x,
         numero_paradas.gris = numero_paradas.y,
         intraestado = as.factor(intraestado),
         interestado = as.factor(interestado),
...
atraso.descarreg.trasp = df.5 |> 
  select(-c(id_data_carga.y, # variável repetida por causa do join
            hora_chegada_cliente, # manter apenas data e hora da agenda
            hora_inicio_descarreg,   # baser para contruir a variável de atraso do cliente
            diferenca_agenda_descarreg, # relevante para base de atraso do cliente
            cliente_atrasou, # relevante para base de atraso do cliente
            hora_fim_descarreg, # baser para contruir a variável delta_carregamento
            parado_ou_viagem,# não trouxe mais informações
            hora_inicio_descarreg,# manter apenas data e hora da agenda
            hora_fim_descarreg,   # manter apenas data e hora da agenda
            cpf_moto,   # poucas observações para manter cada motorista
            id_CPF_MOTO # repedita por causa do join
            )) |>
  filter(!is.na(GR_JORNADA)) |>
